...
class CostCenterViewModel extends GetxController {
  String? editItem;
  ...
  void initModel() {
    allCost.clear();
    List<CostCenterModle> rootList = costCenterModelList.values.toList().where((element) => element.isParent ?? false).toList();
  ...
  CostCenterTree addToModel(CostCenterModle element) {
    var list = element.child?.map((e) => addToModel(costCenterModelList[e]!)).toList();
    CostCenterTree model = CostCenterTree.fromJson({"name": element.name}, element.id, list ?? []);
    return model;
  }
  ...
  addChild({String? parent, name}) async {
    ...
    FirebaseFirestore.instance.collection(Const.costCenterCollection).doc(id).set({
      "name": name,
      if (parent != null) "parent": parent,
      "isParent": parent == null,
    });
    if (parent != null) {
      FirebaseFirestore.instance.collection(Const.costCenterCollection).doc(parent).update({
  ...
  removeChild({String? parent, id}) async {
    FirebaseFirestore.instance.collection(Const.costCenterCollection).doc(id).delete();
    if (parent != null) {
      FirebaseFirestore.instance.collection(Const.costCenterCollection).doc(parent).update({
  ...
  void endRenameChild() {
    FirebaseFirestore.instance.collection(Const.costCenterCollection).doc(editItem).update({
      "name": editCon?.text,
    });
    editItem = null;
    update();
