...
class EntryBondViewModel extends GetxController {
  ...
  prevBond() {
    ...
    if (codeList.values.toList().first == codeList.values.toList()[index]) {
      ...
      tempBondModel = GlobalModel.fromJson(
          allEntryBonds[codeList.values.toList()[index - 1]]?.toFullJson());
  ...
  firstBond() {
    ...
    if (codeList.values.toList().first == codeList.values.toList()[index]) {
      ...
      tempBondModel = GlobalModel.fromJson(
          allEntryBonds[codeList.values.toList().first]?.toFullJson());
      Get.off(() => EntryBondDetailsView(oldId: tempBondModel.entryBondId));
      update();
      initPage();
    }
  ...
  lastBond() {
    ...
    if (codeList.values.toList().last == codeList.values.toList()[index]) {
      ...
      tempBondModel = GlobalModel.fromJson(
          allEntryBonds[codeList.values.toList().last]?.toFullJson());
      Get.off(() => EntryBondDetailsView(oldId: tempBondModel.entryBondId));
      initPage();
      update();
    }
  ...
  void changeIndexCode({required String code}) {
    var model = allEntryBonds.values
        .toList()
        .firstWhereOrNull((element) => element.entryBondCode == code);
    if (model == null) {
