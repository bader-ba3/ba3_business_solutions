...
class InvoiceViewModel extends GetxController {
  final CollectionReference _invoicesCollectionRef = FirebaseFirestore.instance.collection("Invoices");
  ...
  onCellTap(RowColumnIndex rowColumnIndex) {
    if (rowColumnIndex.rowIndex + 1 == records.length) {
      records.add(InvoiceRecordModel());
  ...
  addBills(GlobalModel invoiceModel, List<InvoiceRecordModel> record, isEdit, {bool withLogger = false}) async {
    ...
    if (!invoiceModel.invSeller!.contains("saller")) invoiceModel.invSeller = getSellerIdFromText(invoiceModel.invSeller);
    ...
    if (!invCodeList.contains(invoiceModel.invCode)) {
      ...
      print(patternController.patternModel[invoiceModel.patternId]);
      print(patternController.patternModel[invoiceModel.patternId]!.patHasVat!);
  ...
  buildSource(String billId) {
    records = getInvoiceRecords(billId) + [InvoiceRecordModel()];
    invoiceRecordSource = InvoiceRecordSource(records: records, accountVat: getAccountModelFromId(getAccountIdFromText(secondaryAccountController.text))!.accVat!);
  }
  ...
  buildInvInit(bool bool, String invId) {
    ...
    storeController.text = initModel.invStorehouse!;
  ...
  getAccountComplete() async {
    _accountPickList = [];
    accountController.accountList.forEach((key, value) {
      _accountPickList.addIf(value.accCode!.toLowerCase().contains(_secondaryAccountController.text.toLowerCase()) || value.accName!.toLowerCase().contains(_secondaryAccountController.text.toLowerCase()), value.accName!);
    });
