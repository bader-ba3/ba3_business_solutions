...
class _SliverListWidgetState<T> extends State<SliverListWidget<T>> {
  ...
  Future<void> _fetchPage(pageKey) async {
    ...
    try {
      List<T> newItems = [];
      List<T> Items = widget.allElement;
      if (_pageSize * (turn + 1) < Items.length) {
        if (_searchTerm == null || _searchTerm!.isEmpty) {
          newItems = Items.sublist(_pageSize * turn, _pageSize * (turn + 1));
          isSearch = false;
        } else {
          turn = -1;
          isSearch = true;
          newItems = Items.where((element) => widget.where(element, _searchTerm!)).toList();
        }
      } else {
        if (_searchTerm == null || _searchTerm!.isEmpty) {
          newItems = Items.sublist(_pageSize * turn);
          isSearch = false;
        } else {
          turn = -1;
          isSearch = true;
          ...
          children: [
            Row(
              children: [
                ...
                Text(widget.header,style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 25),),
                const Spacer(),
                Padding(
