...
import 'package:ba3_business_solutions/core/shared/widgets/grid_column_item.dart';
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: GetBuilder<EntryBondController>(builder: (controller) {
        return Scaffold(
          appBar: AppBar(centerTitle: true, title: const Text("سند قيد"), leading: const BackButton(), actions: [
            SizedBox(
              width: 300,
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: DatePicker(
                      initDate:
                          (controller.tempBondModel.bondDate ?? controller.tempBondModel.invDate ?? controller.tempBondModel.cheqDate).toString(),
                      onSubmit: (_) {},
                    ),
                  ),
              ...
              children: [
                  ...
                  children: [
                    ...
                    GetBuilder<BondController>(builder: (controller) {
                      double _ = entryBondController.tempBondModel.entryBondRecord
                          ...
                          0;
                      return Container(
                          color: double.parse("0") == 0 ? Colors.green : Colors.red,
                ...
                if (controller.tempBondModel.invId != null || controller.tempBondModel.bondId != null)
                  AppButton(
                      title: "الأصل",
                      onPressed: () {
                        if (controller.tempBondModel.globalType == AppConstants.globalTypeInvoice) {
                          Get.to(
                            () => InvoiceView(
                        ...
                        } else if (controller.tempBondModel.globalType == AppConstants.globalTypeCheque) {
                          Get.to(() => AddCheque(
                                modelKey: controller.tempBondModel.cheqId,
                              ));
                        } else if (controller.tempBondModel.bondType == AppConstants.bondTypeDaily ||
                            controller.tempBondModel.bondType == AppConstants.bondTypeStart) {
                          Get.to(() => BondDetailsView(
                                oldId: controller.tempBondModel.bondId,
                                bondType: controller.tempBondModel.bondType!,
                              ));
                        } else if (controller.tempBondModel.bondType == AppConstants.bondTypeDebit ||
                            controller.tempBondModel.bondType == AppConstants.bondTypeCredit) {
                          Get.to(() => CustomBondDetailsView(
                                oldId: controller.tempBondModel.bondId,
                                isDebit: controller.tempBondModel.bondType == AppConstants.bondTypeDebit,
                              ));
                        }
                      },
