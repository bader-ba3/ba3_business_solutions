...
class StatisticChart extends StatefulWidget {
  const StatisticChart({
    super.key, required this.allSpots});
  ...
  Widget build(BuildContext context) {
    final lineBarsData = [
      LineChartBarData(
        ...
        showingIndicators: allSpots.map((e) => e.x.toInt(),).toList(),
        spots: allSpots,
        isCurved: true,
        barWidth: 4,
        shadow: const Shadow(
    ...
    return Padding(
      ...
      child: LayoutBuilder(builder: (context, constraints) {
        return LineChart(
          LineChartData(
            showingTooltipIndicators: allSpots.map((index) {
              return ShowingTooltipIndicators([
                LineBarSpot(
                  tooltipsOnBar,
                  lineBarsData.indexOf(tooltipsOnBar),
                  tooltipsOnBar.spots.firstWhere((element) => element.x ==index.x ,),
                ),
              ]);
            }).toList(),
